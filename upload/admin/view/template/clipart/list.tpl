	...
	<div class="page-header">
		<div class="container-fluid">
			<div class="pull-right">
				<a href="<?php echo $add; ?>" data-toggle="tooltip" title="<?php echo $button_add_clipart; ?>" class="btn btn-primary"><i class="fa fa-plus"></i></a>
					...
					<div class="row">
						<div class="col-sm-4">
							<div class="form-group">
								...
								<input type="text" name="filter_id_clipart" value="<?php echo $filter_id_clipart; ?>" placeholder="<?php echo $entry_id; ?>" id="input-id" class="form-control" />
							</div>
						</div>
						<div class="col-sm-4">
							<div class="form-group">
						...
						<table class="table table-bordered table-hover">
							<thead>
								<tr>
									...
									<td class="text-right"><?php echo $column_id_clipart; ?></td>
									<td class="text-left">
										<?php if ($sort == 'c.name') { ?>
							...
							<tbody>
								...
								<tr>
									<td style="text-align: center;">
										<?php if ($clipart['selected']) { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $clipart['id_clipart']; ?>" checked="checked" />
										<?php } else { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $clipart['id_clipart']; ?>" />
										<?php } ?>
									</td>
									<td width="20" class="right"><?php echo $clipart['id_clipart']; ?></td>
									<td class="left"><?php echo $clipart['name']; ?></td>
									<td class="left"><?php echo $clipart['status']; ?></td>
									<td class="left">
										<ul>
											<?php foreach ($clipart['categories'] as $category) { ?>
									...
									<td class="left">
										<ul>
											<?php foreach ($clipart['keywords'] as $keyword) { ?>
											<li><?php echo $keyword; ?></li>
											<?php } ?>
										</ul>
									</td>
									<td align="center" valign="middle"><img src="<?php echo $clipart['thumb']; ?>" /></td>
									<td class="right"><?php echo $clipart['date_added']; ?></td>
									<td class="right">
										<a href="<?php echo $clipart['edit']['href']; ?>" data-toggle="tooltip" title="<?php echo $clipart['edit']['text']; ?>" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
									</td>
								</tr>
									<?php } ?>
								<?php } else { ?>
...
$('#button-filter').on('click', function filter() {
	url = 'index.php?route=clipart/clipart/_list&token=<?php echo $token; ?>';
	
	var filter_id_clipart = $('input[name=\'filter_id_clipart\']').attr('value');
	
	if (filter_id_clipart) {
		url += '&filter_id_clipart=' + encodeURIComponent(filter_id_clipart);
	}
