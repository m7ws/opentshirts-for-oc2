...
<table class="table table-bordered table-hover">
  <thead>
	<tr>
	  <td class="text-center"><input type="checkbox" onclick="$('input[name*=\'data[screenprinting_colors]\']').attr('checked', this.checked);" /></td>
	...
	<?php } else { ?>
		...
		<tr>
			...
			<?php if (!$color['isdefault']) { ?>
				...
				<input type="checkbox" name="data[screenprinting_colors][]" value="<?php echo $color['id_design_color']; ?>" checked="checked" class="form-control" />
				<?php } else { ?>
				<input type="checkbox" name="data[screenprinting_colors][]" value="<?php echo $color['id_design_color']; ?>" class="form-control" />
				<?php } ?>
			<?php } ?>
			</td>
