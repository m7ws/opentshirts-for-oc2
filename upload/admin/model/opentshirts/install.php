...
class ModelOpentshirtsInstall extends Model {
	public function install() {
		...
		$this->db->query("
			...
			('opentshirts', 'opentshirts_max_product_color_combination', '5', 0),
			('opentshirts', 'opentshirts_theme', 'redmond', 0),
			('opentshirts', 'opentshirts_config_logo', 'data/logo_studio.png', 0),
			('opentshirts', 'opentshirts_home_button_link', '', 0);
		");
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET `code` = 'opentshirts_setting', `key` = 'opentshirts_setting_product_size_option_id', `value` = '" . $option_id . "'");
	...
	public function uninstall() {
		...
		if($this->config->get('opentshirts_setting_product_color_option_id')) {
			$this->model_catalog_option->deleteOption($this->config->get('opentshirts_setting_product_color_option_id'));
		}
		if($this->config->get('opentshirts_setting_product_size_option_id')) {
			$this->model_catalog_option->deleteOption($this->config->get('opentshirts_setting_product_size_option_id'));
		}

		$this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `code` = 'opentshirts'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `code` = 'opentshirts_setting'");
