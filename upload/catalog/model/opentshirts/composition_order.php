...
class ModelOpentshirtsCompositionOrder extends Model {
	...
	public function addCompositionToOrderItem($order_product_id, $id_composition) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "composition_to_order_item WHERE id_composition = '" . $this->db->escape($id_composition) . "' AND order_product_id = '" . $this->db->escape($order_product_id) . "' ");
		if(!$query->num_rows) {
			$sql  = "INSERT INTO " . DB_PREFIX . "composition_to_order_item SET id_composition = '" . $this->db->escape($id_composition) . "', order_product_id = '" . $this->db->escape($order_product_id) . "' ";
			$this->db->query($sql);
		}
	}
